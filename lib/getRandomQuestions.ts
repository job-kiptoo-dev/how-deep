...
//   } else {
//     selectedLevel =
//       questionsData.levels[
//         Math.floor(Math.random() * questionsData.levels.length)
//       ];
//   }
//
//   const randomIndex = Math.floor(
//     Math.random() * selectedLevel.questions.length,
//   );
//   return {
...
export function getRandomQuestion(level?: number) {
  ...
  if (!selectedLevel.questions.length) {
    throw new Error(`No questions found for level ${selectedLevel.level}`);
  }
